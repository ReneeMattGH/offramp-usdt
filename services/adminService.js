...
class AdminService {
    ...
    async updateOrderStatus(orderId, status, note, adminId) {
        ...
        if (status === 'approved') {
            await supabase.from('exchange_orders').update({ status: 'APPROVED', updated_at: new Date() }).eq('id', orderId);
            await supabase.from('payout_orders').update({ status: 'APPROVED' }).eq('id', orderId);
        } else if (status === 'success') {
            await supabase.from('exchange_orders').update({ status: 'completed', completed_at: new Date() }).eq('id', orderId);
