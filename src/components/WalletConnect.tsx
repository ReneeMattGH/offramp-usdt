...
export function WalletConnect({ onConnect, className }: WalletConnectProps) {
  ...
  const [manualNetwork, setManualNetwork] = useState<'mainnet' | 'nile'>('nile');
  ...
  const getApiBase = () => {
    return manualNetwork === 'nile' ? 'https://api.nileex.io' : 'https://api.trongrid.io';
  };

  const getUsdtContract = () => {
    return manualNetwork === 'nile' ? 'TXLAQ63Xg1NAzckPwKHvzw7CSEmLMEqcdj' : 'TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t';
  };
    ...
    return (
      <div className={`flex items-center gap-2 px-4 py-2 bg-primary/10 rounded-lg border border-primary/20 ${className}`}>
        ...
        <div className="ml-2 pl-3 border-l border-border flex flex-col items-end">
             ...
             <span className="text-[10px] text-muted-foreground">{showNetwork === 'nile' ? 'Nile Testnet' : showNetwork === 'mainnet' ? 'Mainnet' : 'Unknown'}</span>
        </div>
      </div>
    );
  }
          ...
          {mode === 'tronlink' ? (
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                <Button
                  variant={manualNetwork === 'nile' ? 'default' : 'outline'}
                  onClick={() => setManualNetwork('nile')}
                  size="sm"
                >
                  Nile Testnet
                </Button>
                <Button
                  variant={manualNetwork === 'mainnet' ? 'default' : 'outline'}
                  onClick={() => setManualNetwork('mainnet')}
                  size="sm"
                >
                  Mainnet
                </Button>
              ...
              {manualConnected && (
                <div className="flex items-center justify-between p-3 rounded-lg border">
                  <div className="text-xs">
                    ...
                    <div className="text-muted-foreground">{manualNetwork === 'nile' ? 'Nile Testnet' : 'Mainnet'}</div>
                  </div>
